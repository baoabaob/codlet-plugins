...
const toml = value => typeof value === 'string' ? JSON.stringify(value) : typeof value === 'boolean' || typeof value === 'number' ? String(value) : Array.isArray(value) ? `[${value.map(toml).join(',')}]` : `{${Object.entries(value).filter(([, item]) => item != null).map(([key, item]) => `${JSON.stringify(key)}=${toml(item)}`).join(',')}}`;
function prepareBackendLaunch({ executable, arguments: args, originalEnvironment, environmentPatch, shellPolicy, platform = process.platform }) {
  ...
  return Object.freeze({ executable, arguments: Object.freeze([...args, '-c', filter, '-c', `shell_environment_policy.set=${toml(restored.policy.set)}`]),
...
module.exports = { prepareBackendLaunch, toml };
