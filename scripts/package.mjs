...
const packages=[];
for(const dir of ['codex-ui-adapter','codex-desktop-adapter','codlet']){
  ...
  const dependencies=manifest.id==='codlet-gui'?['codex.ui.adapter']:[];
  ...
  const readme=Buffer.from(`# ${manifest.name}\n\nCodlet first-party plugin, version ${manifest.version}.\n\nPermissions: ${manifest.permissions.join(', ')}\n\nDependencies: ${dependencies.join(', ')||'Core only'}\n\nSource: https://github.com/baoabaob/codlet-plugins\n\nRemove or disable using the Codlet CLI or GUI. Removing the GUI or its dependency from within that GUI is intentionally protected; use the CLI.\n`);
  ...
  packages.push({id:manifest.id,name:manifest.name,version:manifest.version,dependencies,permissions:manifest.permissions,directory:`packages/${manifest.id}`,asset,sha256:hash(archive),bytes:archive.length,files:files.map(([path,bytes])=>({path,bytes:bytes.length,sha256:hash(bytes)}))});
}
