...
for(const plugin of config.plugins){
  ...
  const files=[['codlet.json',manifestBytes],['codlet-package.json',metadata],['README.md',readme],[manifest.renderer.entry,await readFile(resolve(source,manifest.renderer.entry))],...(manifest.host?[[manifest.host.entry,await readFile(resolve(source,manifest.host.entry))]]:[])].sort(([a],[b])=>a.localeCompare(b));
