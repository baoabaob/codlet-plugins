...
class Peer extends EventTarget {
  ...
  send(encoded) {
    ...
    if (method === 'Runtime.evaluate') result = { result: { value: params.expression.startsWith('globalThis[Symbol.for("codlet.private.main-traffic-result.') ? { value: { configuredSessions: 1 } } : true } };
    queueMicrotask(() => {
...
test('main handshake validates exact new process before disclosing launch settings, installs before resume and closes inspector', async () => {
  const args = { inspectorUrl: 'ws://127.0.0.1:12345/01234567-0123-0123-0123-0123456789ab', expectedPid: 1234, executable: process.execPath, configuration: { secretMarker: 'private-proxy-settings' }, WebSocketClass: Peer };
  ...
  assert.equal(JSON.stringify(Peer.commands).includes('private-proxy-settings'), false);
  ...
  assert.equal(result.exactChildVerified, true); assert.equal(result.desktopTrafficVerified, false);
